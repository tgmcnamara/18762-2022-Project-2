...
class Buses:
	...
	def display_grid_data(self, v, file):
		print("BUS ", self.Bus, ":")
		Vm = np.sqrt(v[self.node_Vr]**2 + v[self.node_Vi]**2)
		Va = np.arctan2(v[self.node_Vi], v[self.node_Vr]) * 180.0/np.pi
		print(Vm, " /_", Va)
		file.write(str(self.Bus) + " " + str(Vm) + " " + str(Va) + "\n")
